...
class UniversalTransformer(Transformer):
    ...
    def get_parameters(self, type='meta', named=False):
        if not named:
            ...
            if type == 'meta':
                ...
                prefixs = ['encoder.layers', 'encoder.out', 'encoder.A', 'decoder']
                modules = [self.encoder.layers, self.encoder.out, self.encoder.A, self.decoder]
            
            elif type == 'meta0':
                prefixs = ['encoder.layers', 'encoder.A', 'decoder']
                modules = [self.encoder.layers, self.encoder.A, self.decoder]
    ...
    def save_fast_gradients(self, type='fast'):
        fast_gradients = OrderedDict()
        model_fast_weights = self.get_parameters(type, True)
        for name in model_fast_weights:
            fast_gradients[name] = model_fast_weights[name].grad.data

        with torch.cuda.device(self.gpu):
            return copy.deepcopy(fast_gradients)


    def load_fast_gradients(self, fast_gradients, type='fast'):
        model_fast_weights = self.get_parameters(type, True)
        for name in model_fast_weights:
            model_fast_weights[name].grad.data.copy_(fast_gradients[name])
    ...
    def finite_fast_gradients(self, fast_weights1, fast_weights2, alpha=1):
        fast_weights = OrderedDict()
        model_fast_weights = self.get_parameters('fast', True)
        for name in model_fast_weights:
            model_fast_weights[name].grad.data.add_((fast_weights2[name]-fast_weights1[name]) / alpha)

    def grad_sum(self, grads=None):
        if grads is None:
            weights = self.get_parameters('fast', True)
            return (sum([(weights[name].grad.data ** 2).sum() for name in weights])) ** 0.5
        else:
            return (sum([(grads[name] ** 2).sum() for name in grads])) ** 0.5
